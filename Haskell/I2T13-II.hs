  ...
  case resultEither of
        ...
        Right Response { result = m }  -> do
            ...
            if (Just latestId == upId) then do
                        ...
                        if | T.isPrefixOf "/prefix " t2i -> recvMsg token manager (Just latestId) chatId socket (T.drop 8 t2i) alistMap
                           ...
                           | T.isPrefixOf "/to " t2i -> recvMsg token manager (Just latestId) chatId socket ((Prelude.head . Prelude.tail . T.words $ t2i) <> " :" <> (T.unwords . Prelude.tail . T.words . T.drop 4 $ t2i)) alistMap
                           | "/clear" == t2i -> recvMsg token manager (Just latestId) chatId socket (Prelude.head . T.words $ defaultPrefix) alistMap
