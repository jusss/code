                ...
                [] -> do
                    ...
                    if (not (Just latestId == upId)) then do
                        ...
                        case (D.isPrefixOf "/" t2i) of
                            False -> sendAll socket $ sendToChannel t2i  -- append prefix PRIVMSG
...
sendToChannel x = "PRIVMSG " <> (fst (D.breakSubstring " " x)) <> " :" <> (D.drop 1 (snd (D.breakSubstring " " x))) <> "\r\n"


-- filter prefix messages PRIVMSG QUIT JOIN PART
-- parseMsg :: ByteString -> ByteString
parseMsg x = case (D.isInfixOf " PRIVMSG #" x) of
    True -> (fst (D.breakSubstring ":" (snd $ D.breakSubstring "#" x))) <>  -- channel
...
-- fileter QUIT JOIN PART messages
filterMsg x = ()
        ...
        case D.isPrefixOf "PING" msg of
             ...
             False ->
                   case (parseMsg msg == "hide") of -- filter QUIT JOIN PART messages
                        ...
                        True -> return $ filterMsg msg) -- just return it so we can do nothing with it
