...
recvFromRemote prompt handle recvSock sock id_addr qnames_response enableCache = do
        ...
        either (error . show) id <$> runExceptT do
            ...
            if null _r1 then do
                ...
                when enableCache do
                    let type_list = (fst . fst) <$> _r1
                    -- when CNAME in first, middle or last
                    if CNAME `elem` type_list then do
                        let ((_type, _domain), _rdata) = head _r1
                        let ((_type_, _domain_), _rdata_) = last _r1
                        liftIO $ insert (_type_, _domain) msg <$> (takeMVar qnames_response) >>= putMVar qnames_response
                    else do
