            ...
            case (tail updateId) of
                ...
                x:[] -> do
                    if (not (Just x == upId)) then do
                        ...
                        let t2i = En.encodeUtf8 $ takeIt $ head result
                        case (D.isPrefixOf "/prefix " t2i) of
                             True -> recvMsg token manager (Just x) result chatId socket (D.drop 8 t2i)
                             False -> 
                                          case (D.isPrefixOf "/" t2i) of
                                               False -> sendAll socket $ sendToChannel t2i  defaultPrefix -- append prefix PRIVMSG
                                               True -> sendAll socket (D.drop 1 $ t2i <> "\r\n")  -- raw messages like "/TIME" or "/JOIN #channel", just drop "/" and send the rest to irc server
                        recvMsg token manager (Just x) result chatId socket defaultPrefix

                    else do
                        sleep 1
                        recvMsg token manager (Just x) prevResult chatId socket defaultPrefix
