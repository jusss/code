    ...
    if (L.null fileName) then
        ...
        if | L.any isLeft r1 -> do
           ...
           | otherwise -> do
                ...
                if | (L.length keyList) == (L.length supposeKeyList) -> 
                    if | keyList == supposeKeyList -> do
                           print config
                           ...
                           if | mode == "normal" -> do
                                   irc2T <- async (relayIRC2Tele token manager chatId socket nick nickList userCmd autoJoinChannelCmd parseMsgNormal) -- IRC to Telegram
                                   checkException [t2IRC, pingMsg, irc2T]
                              | mode == "lite" -> do
                                   irc2T <- async (relayIRC2Tele token manager chatId socket nick nickList userCmd autoJoinChannelCmd parseMsgLite) -- IRC to Telegram
                                   checkException [t2IRC, pingMsg, irc2T]
                              | otherwise -> do 
