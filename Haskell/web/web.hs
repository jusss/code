...
main = do
    ...
    scotty 3000 $ do
        ...
        post "/chunk" $ do
            ...
            let cl = read (DTL.unpack $ fromJust contentLength) :: Integer
            liftIO $ print contentLength

            wb <- body -- this must happen before first 'rd'
            {- liftIO $ print wb -}
            {- let filename = "chunk/" <> (DTL.unpack $ DL.last $ DL.take 2 $ DTL.splitOn "\"" $ DL.last $ DTL.splitOn "filename=" $ decodeUtf8 wb) -}
            {- liftIO $ print filename -}
            {- let filename = "chunk/newfile" -}

            {- agent <- header "Upload-Name" -}
            {- let filename = "chunk/" <> (DTL.unpack $ fromJust agent) -}
            headers <- headers
            ...
            {- let step acc lastAcc rl = do  -}
                  {- chunk <- rd -}
                  {- let len = read (show $ D.length chunk) :: Integer -}
                  {- putStr $ "uploaded " <> (show acc) <> ", " -}
                  {- if len >0 -}
                    {- then do -}
                        {- if acc + len > rl -}
                            {- then do -}
                                {- if rl == 0 -}
                                    {- then do  -}
                                        ...
                                        {- step (acc + len) acc 0  -}
                                    {- else do -}
                                        {- D.appendFile filename $ D.take (read (show (rl - lastAcc)) :: Int) chunk -}
                                        {- step (acc + len) acc 0  -}
                            {- else do -}
                      ...
                      {- [> if acc < rl <] -}
                        {- [> then do <] -}
                            ...
                            {- [> step (acc + len) acc rl <] -}
                        {- [> else do <] -}
                            {- [> let l = (read (show (rl - acc)) :: Int) <] -}
                            {- [> if l > 0 <] -}
                                {- [> then D.appendFile filename $ D.take l chunk <] -}
                                {- [> else print "done" <] -}


                    {- else print "read closed" -}
            ...
            {- liftIO $ D.appendFile filename realFileStart -}
            {- let fl = read (show $ D.length realFileStart) :: Integer -}
            {- let rl = cl - fl -}

            lastChunk <- liftIO $ step realFileStart
            {- lastChunk <- liftIO $ step chunk1 -}

            let realFileEnd = fst $ D.breakSubstring "\r\n------WebKitFormBoundary" lastChunk

            liftIO $ D.appendFile filename realFileEnd

            {- liftIO $ D.appendFile filename lastChunk -}

            {- len <- liftIO $ step 0 0 rl -}
