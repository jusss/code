...
class Service:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as response:
                ...
                if response.status == 200:
                        ...
                        try:
                            ...
                            if line:
                                ...
                                if (line == b'data: [DONE]') or (line == b'data: [DONE]\n'):
                                    ...
                                    if "tool_calls" in result["choices"][0]["delta"]:
                                        ...
                                        if tool_call_messages:
                                            ...
                                            for name, parameter, tool_id in fc:
                                                ...
                                                try:
                                                    ...
                                                    yield ('data: ' + json.dumps({"choices": [{"delta": {"content": str(e)}}]})).encode("utf-8"),\
                                                        [], False
    
                                                message.append({"role": "tool", "tool_call_id": tool_id, "name": name, "content": r[:max_input_tokens]})
                                            yield None, message, True
                    ...
                    async for line in response.content:
                        yield ('data: ' + json.dumps({"choices": [{"delta": {"content": line.decode()}}]})).encode("utf-8"),\
                            [], False
    ...
    async def get_answer(self, content, prompt, conversation_id, messages=[]):
        ...
        try:
            ...
            data = {"model": Model, "messages": messages + message, "temperature": 0.7, "top_p": 0.8,
            ...
            async def recursive_tool_call(url, headers, data, conversation_id):
                async for line, processing_messages, tool_call in Service.do_post(url, headers, data, conversation_id):
                    yield line, processing_messages
                    if tool_call:
                        data["messages"] = data["messages"] + processing_messages
                        async for r_line, r_messages in recursive_tool_call(url, headers, data, conversation_id):
                            yield r_line, r_messages

            async for line, _message in recursive_tool_call(url, headers, data, conversation_id):
                if line is None:
            ...
            if message:
                self.conversations[conversation_id] = messages + message
